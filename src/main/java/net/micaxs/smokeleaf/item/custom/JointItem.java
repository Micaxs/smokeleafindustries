...
public class JointItem extends Item {
    ...
    private void applyStoredEffects(ItemStack stack, Player player) {
        ...
        // Grant STONED for the same duration as the longest stored weed effect. The trip shader
        // tier is picked from the strongest THC among the weeds rolled into this joint; high CBD
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context,
        ...
        tooltip.add(Component.literal("Joint Effects:").withStyle(ChatFormatting.GOLD));
        for (int i = 0; i < arr.size(); i++) {
            JsonObject obj = arr.get(i).getAsJsonObject();
            ResourceLocation rl = ResourceLocation.tryParse(obj.get("id").getAsString());
